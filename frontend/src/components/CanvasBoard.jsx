...
const sendPdfToWhatsApp = () => {
  ...
  pdf.addImage(snapshot, "PNG", 0, 0, canvas.width, canvas.height);

  pdf.save("whiteboard-notes.pdf");

  const message = encodeURIComponent(
    "PDF downloaded. Please attach 'whiteboard-notes.pdf' and send it."
  );

  window.open(`https://wa.me/?text=${message}`, "_blank");
};

// ✅ Send current class link to WhatsApp
const sendLinkToWhatsApp = () => {
  const link = window.location.href;

  const whatsappUrl = `https://wa.me/?text=${encodeURIComponent(
    "Join my class here: " + link
  )}`;
        ...
        <div className="flex-1 flex flex-col bg-white rounded-xl shadow-sm overflow-hidden">
          ...
          <div className="bg-gray-50 border-t p-3 flex flex-wrap gap-2 justify-center">
            ...
            <button
              ...
              className="px-3 py-2 text-xs font-semibold rounded-lg bg-green-500 text-white hover:bg-green-600"
            >
              🔗 Share Link
            </button>
