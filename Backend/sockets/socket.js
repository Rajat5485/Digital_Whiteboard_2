...
const initializeSocket = (io) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("toggle-draw-permission", ({ classId, targetSocketId, allowed }) => {
      ...
      // ✅ 1. Send update to student
      io.to(target.socketId).emit("draw-permission-changed", {
        allowed: target.canDraw,
      });

      // ✅ 2. Notify teacher (THIS WAS MISSING)
      socket.emit("permission-notification", {
        message: `${target.userName} ${allowed ? "granted" : "revoked"} draw access`,
        userId: target.userId,
        allowed: target.canDraw,
      });

      // ✅ 3. Broadcast to entire class (best practice)
    ...
    socket.on("raise-hand", ({ classId, raised }) => {
      ...
      current.handRaised =
        raised === undefined ? !current.handRaised : Boolean(raised);
    ...
    socket.on("send-notes-to-selected", async ({ classId, notes, recipientUserIds }) => {
      ...
      const recipients = recipientUserIds
        .map((userId) => {
          const user = Object.values(classSessions[classId] || {}).find(
            (u) => u.userId === userId
          );
          return user ? user.socketId : null;
        })
